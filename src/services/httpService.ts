...
export async function atualizarDespesaBanco(payload: EditarDespesaRequestDto): Promise<EditarDespesaRequestDto | ErroResponseDto> {
  try {
    console.log("Atualizando despesa com os seguintes dados:");
    console.log(payload);
    // Enviando a requisição via axios
    const response = await travelerApi.put(`/despesa/${payload.id}/update`, payload);
    // Se a resposta for 201 (Criado), retorna os dados da despesa
    if (response.status === 201) {
      return response.data;  // Dados da despesa criada
    }
